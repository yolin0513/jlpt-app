...
3. **授權邊界**（本 repo 內，詳見 STATUS §3.2）：
   ...
   - **異動較大**（判定清單見 STATUS §3.3）：先走多代理投票（由統籌者主持，照共用慣例 §1），再動手。
...
4. **跨專案唯讀**：同一層的其他 App（`../TripQuest`、`../StockDiary`、`../MealMate` 等）只能讀、不能改；本專案以外的任何檔案都不要動。
